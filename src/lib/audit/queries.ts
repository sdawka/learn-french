...
export function getSessionTimeline(session_id: number): AuditEvent[] {
  return query<{
    id: number;
    session_id: number;
    event_type: string;
...
export function getScaffoldTrajectory(
  session_id: number
): Array<{ ts: string; direction: string; new_level: number }> {
  return query<{ payload_json: string; ts: string }>(
